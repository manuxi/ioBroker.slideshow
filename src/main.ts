	...
	private async updateCurrentPictureTimer(direction: 1 | -1 = 1): Promise<void>{
		...
		try{
			if (CurrentPictureResult !== null && this.isUnloaded === false){
				...
				// IMPORTANT: write all metadata BEFORE the picture URL.
				// The widget listens on data.oid (the picture url) as the trigger
				// to repaint and then reads info_album/date/info1..3 from the
				// state cache. If url were written first, the widget could race
				// and render info from the previous picture.
				...
				// Write picture LAST — this is the trigger the widget listens on.
